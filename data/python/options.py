...
def getWheelOptions(wheels):
    ...
    for wheel in wheels:
        if(wheel not in total_wheel):
            ...
            total_wheel.append("WheelName="+wheel+",WheelID="+str(r.randint(0,1000000))+",WheelDiameter="+wheelDiameter+",WheelStyle="+wheelStyle+",WheelRunFlat="+str((bool)(r.randint(0,1))))
    return total_wheel
...
def getEngineOptions(engines):
    ...
    for engine in engines:
        if(engine not in total_engine):
            total_engine.append("EngineName="+engine+",EngineID="+str(r.randint(0,1000000))+",EngineLiters="+str(r.random()*6+2)[0:3]+",EngineNumCylinders="+str(r.randint(2,4)*2)+",EngineProductionCost="+str(r.randint(2,8)*1000))
    return total_engine
