...
def main():
    if len(sys.argv) != 10:
        print('Usage: GenerateData -CarFile -BoysFirstNamesFile -GirlsFirstNamesFile -LastNamesFile -StreetFile -CountyTable -StateFile -BusinessNamesFile -number')
    else:
        cars = c.generate(sys.argv[1], sys.argv[9])
        customer = p.generate('Customer', sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[9])
        dealer = p.generate('Dealer', sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[9])
        ...
        for x in range(int(sys.argv[9])):
            ...
            if r.randint(0, 1) == 0:
                ...
                dateOfSale='SaleMonth='+str(r.randint(0,12))+',SaleDay='+str(r.randint(0,28))+',SaleYear='+str(r.randint(2019,2019))
            if r.randint(0, 10) == 0:
                fullData.append(carString+','+inventoryNumber+',SaleID='+str(r.randint(0,1000000))+','+dateOfSale+',CustomerID='+str(r.randint(0,1000000))+',CustomerName='+businesses[x]+','+addresses[x]+','+cd.getPhone('Customer')+',DealerID='+str(r.randint(0,1000000))+','+dealer[x]+',SupplierID='+str(x)+','+supplier[x]+',ManufacturerID='+str(r.randint(0,1000000))+','+manufacturer[x])
            else:
                fullData.append(carString + ','+inventoryNumber+',SaleID='+str(r.randint(0,1000000))+','+dateOfSale+',CustomerID='+str(r.randint(0,1000000))+',' + customer[x] + ','+cd.getPhone('Customer')+ ',DealerID=' +str(r.randint(0,1000000))+','+dealer[x]+',SupplierID='+str(r.randint(0,1000000))+','+supplier[x]+',ManufacturerID='+str(r.randint(0,1000000))+','+manufacturer[x])
